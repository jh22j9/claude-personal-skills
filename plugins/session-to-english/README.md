...
| Q&A 선택 (선택) | 출력할 Q&A 개수 제한 방식 | `random 10`, `max 5` |

### Q&A 선택 옵션

토큰 절약을 위해 **시간대별 파일 샘플링** 방식을 사용합니다. 전체 파일을 읽지 않고, 하루를 구간으로 나눠 구간당 1개 파일만 선택해 읽습니다.

- 파일 15개 → 3구간(오전/오후/저녁) → 3파일 읽기 → 각 파일에서 3~4개
